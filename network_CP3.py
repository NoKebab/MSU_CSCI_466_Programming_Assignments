...
class Router:
    ...
    def forward_packet(self, p, i):
        try:
            ...
            for i in range(len(self.frwdTbl)):
                intf = list(self.frwdTbl[i].keys())[0]
                x = list(self.frwdTbl[i].values())[0]
                key = list(x.keys())[0]
                if dst == key:
                    interface = intf
    ...
    def update_routes(self, p, i):
        ...
        self.formatFtable()
        if self.rt_tbl_D != p:
            print(self.name+' RESENDING')
            for i in range(len(self.intf_L)-1):
                self.send_routes(i)
        else: print(self.name+' FINISHED')
        print(self.name+' FTABLE: ',self.frwdTbl)
